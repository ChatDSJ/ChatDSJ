...
class TestResponsesAPI(unittest.TestCase):
    def setUp(self):
        ...
        self.event = {
            ...
            "text": "<@TESTBOT123> what day is it?"
        }
        ...
        self.mock_messages = [
            ...
            {"user": "U12345", "text": "<@TESTBOT123> what day is it?", "ts": "1617983980.000100"}
        ]
    ...
    def test_responses_api_date_query(self):
        ...
        with patch('app.slack.app.bot_user_id', 'TESTBOT123'):
    ...
    def test_responses_api_full_flow(self):
        ...
        with patch('app.slack.app.bot_user_id', 'TESTBOT123'), \
             patch('app.slack.app.get_openai_response') as mock_get_openai_response:
            mock_get_openai_response.return_value = ("Today is Sunday, April 14, 2025.", {"prompt_tokens": 100, "completion_tokens": 50, "total_tokens": 150})
            ...
            self.assertEqual(call_text, "Today is Sunday, April 14, 2025.")
